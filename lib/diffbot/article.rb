...
# a single entry point: the `.fetch` method, that, given a URL, will return the
# article as analyzed by Diffbot.
class Diffbot::Article < Hashie::Trash
  # Public: Fetch an article from a URL.
  #
  # url      - The article URL.
  # token    - The API token for Diffbot.
  # parser   - The callable object that will parse the raw output from the API.
  #            Defaults to Yajl::Parser.method(:parse).
  ...
  #
  #   # Pass options to the request. See Diffbot::Article::RequestParams to see
  ...
  # Public: List of media items related to the articles. Each item is an object
  ...
  # chunks of HTML which are considered unrelated by Diffbot, unless you set the
  # `dont_strip_ads` option in the request.
  #
  ...
  # confidence - The confidence of Diffbot that the returned text is really the
  #              text of the article. Between 0.0 and 1.0.
  #
  # Only present if you set `stats` to true in the request.
  property :stats

  # Public: The XPath selector at which the body of the article was found in the
  # page.
  property :xpath
  ...
  def media=(raw)
    self["media"] = raw.map { |item| MediaItem.new(item) }
  end
  ...
  # This represents the parameters you can pass to Diffbot to configure a given
  # request. These are either set globally with Diffbot.article_defaults or on a
  # request basis by passing a block to Diffbot::Article.fetch.
  #
  # Example:
  #
